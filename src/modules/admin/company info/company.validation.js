...
export const updateCompanyInfoSchema = Joi.object({
    ...
    'string.empty': 'Arabic company name is required',
  ...
  nameEn: Joi.string().trim().max(100).required().messages({
    ...
    'string.empty': 'English company name is required',
  ...
  phoneNumbers: Joi.array().items(
    Joi.object({
      number: Joi.string().trim().max(20).pattern(phoneRegex).required().messages({
